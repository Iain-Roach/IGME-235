...
function searchButtonClicked(){
    ...
    // 7 - if there's no term to earch then bail out of the function (return does this)
    if(term.length < 1) return;

    // 8 - append the search term to the URL - the parameter name is `q`
    url += "&q=" + term;

    // 9 - grab the user cosen search `limit` from the <select> and append it to the URL
...
function dataLoaded(e) {
    ...
    // 16 - all done bulding the HTML - show it to the user!
    document.querySelector("#content").innerHTML = bigString;

    // 17 - update the status
    document.querySelector("#status").innerHTML = "<b>Sucess!</b><p><i>Here are " + results.length + " results for '" + displayTerm + "'</i></p>";
}
